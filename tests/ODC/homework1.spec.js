...
async function fillRegisterForm(page, locators, testData) {
    ...
    await page.locator(locators.occupationDropDown).selectOption({ label: testData.occupation });
...
test.only('Register, Login, and Validate Product', async ({ browser }) => {
    ...
    const testData = {
        ...
        occupation: 'Student',
        password: faker.internet.password({ length: 12, memorable: true, pattern: /[a-z]/, prefix: '!Q1' }),
    };
    ...
    await expect(page.locator(rahulshettyacademyLocators.registerForm.firstNameField)).toBeVisible();

    // Step 3: Fill and Submit Registration Form
    await fillRegisterForm(page, rahulshettyacademyLocators.registerForm, testData);
    await page.locator(rahulshettyacademyLocators.registerForm.registerButton).click();

    // Step 4: Verify Registration Success and Navigate to Login
    console.log(`Registered with Email: ${testData.email}`);
    console.log(`Password: ${testData.password}`);
    await page.locator(rahulshettyacademyLocators.registerForm.goToLoginButton).click();
    await expect(page.locator(rahulshettyacademyLocators.loginForm.userEmailField)).toBeVisible();

    // Step 5: Login with Registered Credentials
    await page.locator(rahulshettyacademyLocators.loginForm.userEmailField).fill(testData.email);
    await page.locator(rahulshettyacademyLocators.loginForm.userPasswordField).fill(testData.password);
    await page.locator(rahulshettyacademyLocators.loginForm.loginButton).click();

    // Step 6: Verify Products are Displayed
    const firstProduct = await page.locator(rahulshettyacademyLocators.products.productName).nth(0).textContent();
    console.log(`First Product Name: ${firstProduct}`);
    await expect(page.locator(rahulshettyacademyLocators.products.productName).nth(0)).toBeVisible();
